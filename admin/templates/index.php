...
<head>
    ...
    <style>
        ...
        :root { --primary-color: #0d6efd; --secondary-color: #6c757d; --success-color: #198754; --light-bg: #f8f9fa; --border-color: #dee2e6; --text-dark: #212529; --text-muted: #6c757d; --body-bg: #f0f2f5; }
        body { font-family: 'Poppins', sans-serif; background-color: var(--body-bg); padding: 40px 15px; }
        .main-container { background-color: #ffffff; padding: 40px; border-radius: 16px; box-shadow: 0 10px 40px rgba(0, 0, 0, 0.1); width: 100%; max-width: 950px; margin: auto; }
        .header-section h1 { font-weight: 700; color: var(--text-dark); margin-bottom: 8px; }
        .header-section p { font-size: 1.1rem; color: var(--text-muted); max-width: 600px; margin: 0 auto 30px auto; }
        .panel { padding: 30px; background-color: #fff; border-radius: 12px; border: 1px solid var(--border-color); margin-bottom: 30px; }
        .panel-title { font-size: 1.5rem; font-weight: 600; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; gap: 10px; }
        .form-label { font-weight: 500; margin-bottom: 8px; }
        #candidatesSelection { margin-top: 25px; }
        .candidate-list-box { border: 1px solid var(--border-color); border-radius: 8px; max-height: 250px; overflow-y: auto; padding: 10px; }
        .candidate-item { display: flex; align-items: center; padding: 12px 8px; border-bottom: 1px solid #f1f1f1; transition: background-color 0.2s; }
        ...
        .btn-action { padding: 12px; font-size: 1.1rem; font-weight: 600; border-radius: 8px; }
        .candidate-card { background-color: var(--light-bg); border: 1px solid var(--border-color); padding: 20px; margin-bottom: 15px; border-radius: 12px; transition: all 0.3s ease; }
        .candidate-card:hover { transform: translateY(-5px); box-shadow: 0 8px 25px rgba(0, 0, 0, 0.08); }
        .rank-badge { width: 40px; height: 40px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 1.2rem; margin-right: 15px; }
        .rank-1 { background-color: #ffc107; } .rank-2 { background-color: #adb5bd; } .rank-3 { background-color: #cd7f32; } .rank-other { background-color: var(--secondary-color); }
        ...
        .info-box { display: none; background-color: #e9f5ff; border: 1px solid #b6daff; border-radius: 8px; margin-top: 20px; padding: 15px; }
...
<body>
    <div class="main-container">
        ...
        <!-- Panel Kontrol Utama (DIPERBARUI) -->
        <div class="panel">
        ...
        <div class="panel" id="resultsPanel" style="display:none;">
             ...
             <button id="saveResultsButton" class="btn btn-success w-100 mt-3"><i class="fas fa-save"></i> Simpan Semua Hasil</button>
    ...
    $(document).ready(function() {
        ...
        parentBidangFilter.on('change', async function() {
            ...
            resultsPanel.hide();
            infoBox.hide();
            yearFilter.prop('disabled', !currentActiveParentId);
            matchSelectedButton.prop('disabled', true); // Tombol hitung nonaktif sampai sub-bidang dipilih
                        ...
                        const cardHtml = `
                            <div class="candidate-card ${rankClass}">
                                <div class="card-header-result">
                                    ...
                                    <span class="candidate-score">${candidate.skor_kecocokan.toFixed(2)}</span>
                                </div>
        ...
        // PERBAIKAN: Implementasi logika untuk tombol Simpan Hasil
        saveResultsButton.on('click', async function() {
            if (lastMatchingResults.length === 0) {
                saveMessage.text('Tidak ada hasil untuk disimpan.').css('color', 'orange');
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Kosongkan hasil setelah berhasil disimpan untuk mencegah simpan ganda
                    lastMatchingResults = []; 
                } else {
                    saveMessage.text(`Gagal: ${data.error || 'Terjadi kesalahan.'}`).css('color', 'red');
                }
            } catch(error) {
                console.error('Error saving results:', error);
                saveMessage.text('Gagal menyimpan: Kesalahan koneksi.').css('color', 'red');
            } finally {
